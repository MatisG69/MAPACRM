...
export function Modal({ isOpen, onClose, title, children, size = 'md' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center md:items-center p-0 md:p-4 pt-[env(safe-area-inset-top)] md:pt-4">
      <div className="absolute inset-0 bg-black/75 backdrop-blur-sm" onClick={onClose} aria-hidden />
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="modal-title"
        className={`relative rounded-t-[1.75rem] md:rounded-[1.5rem] border border-ws-line bg-ws-panel/98 backdrop-blur-xl shadow-dock w-full ${sizeClasses[size]} max-md:max-w-none max-h-[min(92dvh,calc(100dvh-env(safe-area-inset-top)-0.5rem))] md:max-h-[90vh] flex flex-col mt-auto md:mt-0 pb-[max(0.75rem,env(safe-area-inset-bottom))] md:pb-0`}
      >
        ...
        <div className="overflow-y-auto flex-1 overscroll-contain scrollbar-ws px-4 sm:px-6 py-4 sm:py-5">
          {children}
        </div>
      </div>
    </div>
